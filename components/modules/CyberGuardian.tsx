...
const CyberGuardian: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-on-surface">AI CyberGuardian</h1>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card header={<h2 className="text-xl font-semibold">Network Analysis</h2>}>
            <div className="space-y-4">
              ...
              <textarea
                ...
                rows={5}
                className="w-full p-2 border rounded-md bg-surface-variant/60 dark:bg-surface-variant/30 border-outline"
        ...
        <div>
          <Card header={<h2 className="text-xl font-semibold">Security Report</h2>}>
            ...
            {scanResult && (
              <div className="space-y-4">
                 <div className="text-center p-4 bg-secondary-container text-on-secondary-container rounded-lg">
                    <p className="text-sm font-medium">Overall Security Score</p>
                    <p className="text-5xl font-bold">{scanResult.score}/100</p>
                </div>
                {scanResult.vulnerabilities.map((vuln, index) => (
                    <div key={index} className={`p-3 border-l-4 rounded-r-md ${getRiskColor(vuln.risk)}`}>
                        <h3 className="font-bold text-on-surface-variant">{vuln.title} <span className="text-sm font-medium">({vuln.risk} Risk)</span></h3>
                        <p className="text-sm text-on-surface-variant/80 mt-1">{vuln.description}</p>
                        <p className="text-sm text-green-700 dark:text-green-300 mt-2 font-medium"><strong>Action:</strong> {vuln.recommendation}</p>
                    </div>
                ))}
              </div>
            )}
            {!isLoading && !scanResult && <p className="text-center text-on-surface-variant">Run a scan to see the report.</p>}
          </Card>
        </div>
      </div>
    </div>
